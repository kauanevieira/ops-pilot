...
- [X] T012 [P] Definir os dados da linha de base em `src/store/seed.json`: 5 serviços (`checkout`, `payments`, `auth`, `search`, `notifications`) e exatamente 6 alertas — `alert-1` checkout/critical/firing, `alert-2` payments/high/firing, `alert-3` auth/medium/firing, `alert-4` search/low/resolved, `alert-5` notifications/medium/resolved, `alert-6` checkout/high/resolved — e 0 incidentes. `src/store/seed.ts` lê o arquivo com `readFileSync` e valida contra os esquemas zod do domínio (datas coagidas de string ISO). Identificadores e timestamps determinísticos, para que reaplicar produza estado idêntico (FR-020, FR-021, R-008)
...
- [X] T029 [US2] Implementar o grafo em `src/agents/plan-and-execute.ts` com `StateGraph` e três nós — `planner` → `executor` → `replanner` — com estado (`PEState`) carregando `input`, `plan: string[]` (passos restantes), `done: [step, result][]` (acumulado — seu comprimento é o contador de passos), `answer: string` e `trace`; `name = "plan-and-execute"`. Planner usa `withStructuredOutput<Plan>()`; replanner usa `withStructuredOutput<Replan>()` e devolve uma decisão explícita (`ajustar` | `seguir` | `encerrar`) dos esquemas de T028 (FR-024 a FR-027, R-005) (depende de T017, T018, T019, T020, T028)
...
- [X] T043 Executar o roteiro completo de [quickstart.md](./quickstart.md) com credenciais reais do OpenRouter (`openrouter/free`). Validado ao vivo: ReAct leitura e escrita (abrir+resolver incidente), Plan-and-Execute com revisão de plano e múltiplas tool calls num só passo, limite de iterações em ambas as estratégias (encerramento controlado com rastro parcial, sem exceção), estratégia inválida e credencial ausente. Durante a validação, corrigido um bug real: o wrap-up do sub-agente do `executor` era rotulado `answer` pelo `messagesToTrace`, duplicando o evento de resposta final no rastro do Plan-and-Execute — agora relabelado para `thought`. **Não exercitados ao vivo**: o teto natural de 8 passos (exigiria forçar várias revisões de plano, caro e lento neste modelo gratuito) e a segunda metade da Validação 5 (comparação `react,plan-and-execute` na mesma invocação) — a cota diária do modelo gratuito (`429 free-models-per-day`) esgotou no meio do teste. O caminho do teto de 8 passos usa a mesma lógica de captura de estado parcial já validada pelo teste de `--max-iterations`; recomenda-se reexecutar quando a cota resetar ou com um modelo pago.
